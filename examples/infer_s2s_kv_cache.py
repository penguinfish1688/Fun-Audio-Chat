...
def infer_multiturn_example(model_path, audio_paths):
    """
    Multi-turn S2S inference example.
    This version always enables KV cache during generation.

    Args:
        model_path: Model path
        audio_paths: List of wav paths (user/assistant turns). Assistant GT turns are ignored.
    """
    ...
    # Force KV cache at model/config/generation levels.
    ...
    for index, audio_path in enumerate(audio_paths):
        # Ignore assistant ground-truth audio in input list.
        if index % 2 == 1:
            continue
        ...
        # Add assistant response text into history.
